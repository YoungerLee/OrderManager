...
<html>
  <head>
  	<script type="text/javascript">
  		function changeNum(id,obj,oldnum){
  			...
  			window.location.href="${pageContext.request.contextPath}/sys/changeCart.do?id="+id+"&buynum="+obj.value;
  		}
  	</script>
  </head>
  <body style="text-align: center;" >
  	<h1>我的购物车</h1>
  	<div align="right">
  		<a href="${pageContext.request.contextPath}/sys/prodList.do?thispage=1">继续购物</a>
  		<a href="${pageContext.request.contextPath}/sys/clearCart.do">清空购物车</a>
  	...
  	<c:if test="${empty sessionScope.cartmap}">
  		<h2><a href="${pageContext.request.contextPath}/sys/prodList.do?thispage=1">购物车空空如也,请先去挑点东西吧~~~</a></h2>
  	</c:if>
  	<c:if test="${not empty sessionScope.cartmap}">
		<table width="100%" border="1" style="text-align: center">
			...
			<c:forEach items="${sessionScope.cartmap}" var="entry">
				<tr>
					<td><img src="${pageContext.request.contextPath}/sys/prodImg.do?imgurl=${entry.key.imgurls }"/></td>
					...
					<td>
						<a href="${pageContext.request.contextPath}/sys/delCart.do?id=${entry.key.id }">删除</a>
					</td>
				</tr>
			</c:forEach>
		</table>
