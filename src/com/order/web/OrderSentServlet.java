...
public class OrderSentServlet extends HttpServlet {
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<OrderListForm> list = service.findOrdersSent(id);
		// 3.存入request域带到页面显示
		request.setAttribute("list", list);
		request.getRequestDispatcher("/orderSent.jsp").forward(request,
				response);
	}
